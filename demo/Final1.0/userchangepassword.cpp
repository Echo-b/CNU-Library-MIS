...
{
    if(ui->newPassword->text() == ui->renewPassword->text()){//设置新密码
    ...
    else if(QString(ui->newPassword->text()).toUtf8().toStdString() == ""){ // 未输入新密码
        QMessageBox message(QMessageBox::Critical, "提示","未输入新密码！");
        QIcon *icon = new QIcon(":/image/CNUlibrary.jpg");
        message.setWindowIcon(*icon);
        message.exec();
    }
    else if(QString(ui->renewPassword->text()).toUtf8().toStdString() == ""){ // 未输入确认密码
